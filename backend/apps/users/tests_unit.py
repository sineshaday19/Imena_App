...
class UserAddFormUnitTests(SimpleTestCase):
    def test_password_mismatch_sets_password2_error(self):
        ...
        # ModelForm normally checks DB uniqueness; override to keep this a pure unit test.
...
class JwtAuthUnitTests(SimpleTestCase):
    ...
    def test_unknown_user_rejected(self, mock_filter):
        ...
        s = CustomTokenObtainPairSerializer(
            context={"request": Mock()},
            data={"username": "unknown@test.com", "password": "x"},
        )
        with self.assertRaises(Exception):
            # SimpleJWT raises AuthenticationFailed; Exception is enough for a unit test here.
            s.is_valid(raise_exception=True)

    @patch("apps.users.jwt_auth.authenticate")
    @patch("apps.users.jwt_auth.User.objects.filter")
    @patch("rest_framework_simplejwt.serializers.TokenObtainPairSerializer.validate")
    def test_login_success_resolves_identifier_and_calls_super_validate(
        self, mock_super_validate, mock_filter, mock_authenticate
    ):
        user = Mock(username="resolved-username")
        # First filter(email__iexact=raw) returns a user
