...
class ReportTests(TestCase):
    ...
    def test_contributions_summary(self):
        Contribution.objects.create(rider=self.rider, cooperative=self.coop, date=date(2026, 2, 26), amount=5000, status=Contribution.Status.VERIFIED)
        self._auth_admin()
        resp = self.client.get('/api/reports/contributions-summary/')
    ...
    def test_income_by_cooperative(self):
        IncomeRecord.objects.create(rider=self.rider, cooperative=self.coop, date=date(2026, 2, 26), amount=3000)
        self._auth_admin()
